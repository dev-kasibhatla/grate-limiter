...
fn main() {
    ...
    engine.upsert_provider(ProviderConfig {
        name: "openai".into(),
        quotas: vec![
            QuotaConfig { dimension: Dimension::Requests, limit: 500, window: Some(Window::Minute) },
    ...
    engine.upsert_provider(ProviderConfig {
        name: "anthropic".into(),
        quotas: vec![
            QuotaConfig { dimension: Dimension::Requests, limit: 300, window: Some(Window::Minute) },
            QuotaConfig { dimension: Dimension::Tokens, limit: 80000, window: Some(Window::Minute) },
        ],
        priority: 8,
        weight: 1.0,
        cooldown_seconds: 30,
    ...
    engine.upsert_provider(ProviderConfig {
        name: "gemini".into(),
        quotas: vec![
            QuotaConfig { dimension: Dimension::Requests, limit: 200, window: Some(Window::Minute) },
        ],
        priority: 6,
        weight: 1.0,
        cooldown_seconds: 30,
    ...
    engine.upsert_capability(CapabilityConfig {
        ...
        providers: vec![
            ...
            CapabilityProvider { provider: "gemini".into(), priority: 6 },
        ],
    });
    ...
    for i in 0..50 {
        ...
        println!("[{:3}] -> {} (score: {:.3})", i, decision.provider, decision.score);
        ...
        engine.observe(Observation {
            ...
            usage: Usage { requests: 1, tokens: Some(500), ..Default::default() },
            outcome: Outcome { status, latency_ms: 200 },
        }).unwrap();
    }
    ...
    for provider in ["openai", "anthropic", "gemini"] {
        ...
        let remaining = engine.provider_quota_remaining(provider, Dimension::Requests).unwrap_or(0);
        println!("{provider:>10}: health={health:.2} cooldown={cooldown} remaining_requests={remaining}");
    }
